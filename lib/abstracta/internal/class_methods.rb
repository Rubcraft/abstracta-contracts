...
module Abstracta
  module Internal
    ...
    def implements(*interfaces)
      ...
      normalized.each { |interface| include(interface) unless ancestors.include?(interface) }
      self
    end
    ...
    def validate_implementation!
      ...
      missing_instance = missing_methods
      missing_class = missing_class_methods

      unless missing_instance.empty? && missing_class.empty?
        raise Internal::UnimplementedMethodsError.new(
          self,
          instance_methods: missing_instance,
          class_methods: missing_class
        )
      end
